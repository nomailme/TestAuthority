...
public class CertificateRequestValidator : AbstractValidator<CertificateRequestModel>
{
    ...
    {
        RuleFor(x => x).Must(AnyHostnamesOrIpAddresses).WithMessage("You must provide at least one hostname or IP address.");
        RuleFor(x => x.Password).MinimumLength(1).When(x => x.Format == CertificateFormat.Pfx).WithMessage("You must provide a password for PFX");
    }
    ...
    private bool AnyHostnamesOrIpAddresses(CertificateRequestModel request)
    {
        ...
        return sanRecords > 0 ? true : false;
    }
